...
def pose_callback(msg):
    ...
    if msg.status.status == 3 and count>0 :  #成功到达任意目标点，前往下一目标点
        ...
        if index == count:                   #当index等于count时，表示所有目标点完成，重新开始巡航
            ...
            pose.target_pose.pose.position.x = markerArray.markers[index].pose.position.x
            pose.target_pose.pose.position.y = markerArray.markers[index].pose.position.y
            pose.target_pose.pose.orientation.z = markerArray.markers[index].pose.orientation.z
            pose.target_pose.pose.orientation.w = markerArray.markers[index].pose.orientation.w
...
def send_mark():
    ...
    goal_pub    = rospy.Publisher('/move_base_flex/move_base/goal', move_base_msgs.MoveBaseActionGoal, queue_size = 1) #用于发布目标点
    send_flag=Int8()
    send_flag.data=1
    ...
    goal_status_sub = rospy.Subscriber('/move_base_flex/move_base/result', move_base_msgs._MoveBaseActionResult, pose_callback) #用于订阅是否到达目标点状态
