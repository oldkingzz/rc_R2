...
def send_yunxin_msg(vx,vy,angular):
    ...
    #data = Chassis_Data_Rx(0, 0,0.4)
    #message = build_chassis_message(data)
    #ser.write(message)
    #time.sleep(3)
    #data = Chassis_Data_Rx(0, 0.2,0)

    #message = build_chassis_message(data)
    #ser.write(message)
    #time.sleep(3)
    #data = Chassis_Data_Rx(0, 0,0)
#     #rospy.init_node("Sub_node")
#     #sub = rospy.Subscriber("/cmd_vel",Twist,Sub_callback,queue_size=10)
#  #  data = T(Twist)
    #message = build_chassis_message(data)
...
def listener():
    ...
    # 创建一个订阅器，订阅/cmd_vel主题，注册回调函数callback
