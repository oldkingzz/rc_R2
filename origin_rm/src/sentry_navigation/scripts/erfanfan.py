#!/usr/bin/env python
# -*- coding: utf-8 -*-

'''
@File    :   tran_nuc_stm32.py
@Time    :   2024/03/11 08:16:44
@Author  :   33 
@Version :   1.0
@Desc    :   把数据从nuc传到stm32
'''

import rospy
from geometry_msgs.msg import Twist
from std_msgs.msg import Bool
import struct
import serial
import time

# CRC-8 校验表
CRC08_Table = [
    ,
]

class Chassis_Data_Rx:
    def __init__(self, status):
        self.status = status

class T:
    def __init__(self, vel_msg):
        self.vx = vel_msg.linear.x
        self.vy = vel_msg.linear.y
        self.vz = vel_msg.linear.z

# 定义帧头和命令字
HEADER = 0xAA
CMD_ID_CHASSIS_DATA_RX = 0x83

# 打包结构体为字节流
def pack_chassis_data(data):
    return struct.pack('<f', data.status)

# 计算CRC校验位
def crc8(data):
    crc = 0xff
    for byte in data:
        crc = CRC08_Table[crc ^ byte]
    return crc

# 构建消息
def build_chassis_message(data):
    data_bytes = pack_chassis_data(data)
    length = len(data_bytes) + 4  # 包含帧头、帧长度、命令字和校验位
    crc = crc8(struct.pack('<BBB', HEADER, length, CMD_ID_CHASSIS_DATA_RX) + data_bytes)
    return struct.pack('<BBB', HEADER, length, CMD_ID_CHASSIS_DATA_RX) + data_bytes + struct.pack('<B', crc)

# 连接串口
ser = serial.Serial('/dev/ttyUSB0', 115200)  



def handaoshan_callback(msg):
    global status
    status = 1 if msg.data else 0
    print("Received message with data: %d" % status)  # Debug message

def send_yunxin_msg(status):
    data = Chassis_Data_Rx(status)
    message = build_chassis_message(data)
    ser.write(message)
    print("Sent message with status: %d" % status)

def listener():
    # 初始化节点
    rospy.init_node('status_listener')
    rospy.Subscriber('handaoshan', Bool, handaoshan_callback)
    rate = rospy.Rate(10)  # Adjust the frequency as needed
    while not rospy.is_shutdown():
        send_yunxin_msg(status)  # Continuously send the current status
        rate.sleep()

if __name__ == "__main__":
    status = 0  # Default status as 0
    listener()